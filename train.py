...
## VQGates
parser.add_argument('--vq_gate', action='store_true')
parser.add_argument('--num_embeddings', type=int, default=512)
parser.add_argument('--contrib_rate', type=float, default=0.1)
parser.add_argument('--beta_vq', type=float, default=1.0)
parser.add_argument('--vqtype', type=str, default='VQ Method')
parser.add_argument('--one_gate', action='store_true')
...
if args.restart:
    ...
    model = MemTransformerLM(ntokens, args.n_layer, args.n_head, args.d_model,
        ...
        dense_drop=args.dense_drop, expert_drop=args.expert_drop, num_expert=args.num_expert, attn_moe=args.attn_moe, num_embeddings=args.num_embeddings, vq_gate=args.vq_gate, contrib_rate=args.contrib_rate, vqtype=args.vqtype, one_gate=args.one_gate)
...
def train():
    ...
    for batch, (data, target, seq_len) in enumerate(train_iter):
            ...
            for i in range(args.batch_chunk):
                ...
                if args.vq_gate:
                    ...
                    for name, m in model.named_modules():
                        if isinstance(m, VQMoE):
                            vq_loss += m.loss
                    vq_loss = args.beta_vq * vq_loss
                #bl loss
                ...
                if args.fp16:
                    #optimizer.backward(loss+vq_loss+balance_loss)
                    if args.vq_gate and args.load_balance > 0:
                        optimizer.backward(loss+vq_loss+balance_loss)
                    elif args.vq_gate:
                        optimizer.backward(loss+vq_loss)
                    elif args.load_balance > 0:
                        optimizer.backward(loss+balance_loss)
                    else:
                        optimizer.backward(loss)
                else:
                    #(loss+vq_loss+balance_loss).backward()
                    #loss.backward()
                    if args.vq_gate and args.load_balance:
                        (loss+vq_loss+balance_loss).backward()
                    elif args.vq_gate:
                        (loss+vq_loss).backward()
                    elif args.load_balance > 0:
        ...
        if train_step % args.log_interval == 1:
            ...
            try:
                vq_loss_vl = vq_loss.item()
            except:
                vq_loss_vl = -1.0
            #balance loss
            if isinstance(balance_loss, float):
                balance_loss_vl = balance_loss
            else:
                balance_loss_vl = balance_loss.item()
            log_str = '| epoch {:3d} step {:>8d} | {:>6d} batches | lr {:.3g} ' \
                      '| ms/batch {:5.2f} | loss {:5.2f} | vqloss: {:5.2f} | blloss: {:5.2f}'.format(
                epoch, train_step, batch+1, optimizer.param_groups[0]['lr'],
                elapsed * 1000 / args.log_interval, cur_loss, vq_loss_vl, balance_loss_vl)
