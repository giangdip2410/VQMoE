...
class FMoE(nn.Module):
    ...
    * `expert` can be specified as a module class, it is used to generate
    ...
    def __init__(
        ...
        if vq_gate:
            pass
            #self.gate = gate(d_model=d_model, num_expert=num_expert, world_size=world_size, top_k=top_k, num_embeddings=num_embeddings)
        else:
    ...
    def forward(self, moe_inp, all_gate=None, top_k=None):
        ...
        if top_k is not None:
            org_topk = self.top_k
            self.top_k = top_k
            gate_top_k_idx, gate_score = self.gate(moe_inp, top_k=top_k)
        else:
